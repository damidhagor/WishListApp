...
internal sealed class IdentityNoOpEmailSender : IEmailSender<WishlistUser>
{
    private readonly IEmailSender _emailSender = new NoOpEmailSender();

    public Task SendConfirmationLinkAsync(WishlistUser user, string email, string confirmationLink) =>
    ...
    public Task SendPasswordResetLinkAsync(WishlistUser user, string email, string resetLink) =>
        _emailSender.SendEmailAsync(email, "Reset your password", $"Please reset your password by <a href='{resetLink}'>clicking here</a>.");

    public Task SendPasswordResetCodeAsync(WishlistUser user, string email, string resetCode) =>
